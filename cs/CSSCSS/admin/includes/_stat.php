...
<table class="table table-striped"> <thead><th>Ник игрока</th><th>Причина</th><th>Статус</th><th>IP</th><th>Сервер</th><th>Действие</th></thead><tbody>

<?
do{
if($myrow["ban"] === "0") {$b = "Не расмотрена"; $b1 = "class='active'";} if($myrow["ban"] === "1") {$b = "Рассмотрена<br> (<font color='Firebrick'>не разбанен</font>)"; $b1 = "class='danger'";} if($myrow["ban"] === "2") {$b = "Рассмотрена<br> (<font color='green'>разбанен</font>)"; $b1 = "class='success'";}
$serv_id = $myrow["server"];
if($serv_id>0){
$result2 = mysql_query("SELECT * FROM server WHERE id='$serv_id'");
$myrow2 = mysql_fetch_array($result2);
$server = '<a href="?go=server&view='.$serv_id.'">'.$myrow2['name'].'</a>';
} else {$server = '<b>Сервер не выбран</b>';}
printf ("<tr $b1 >
...
<center> <table class="table table-striped"> <thead><th>Ник игрока</th><th>Причина</th><th>Статус</th><th>IP</th><th>Сервер</th><th>Действие</th></thead><tbody>
